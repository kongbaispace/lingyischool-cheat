...
auto framebuffer_size_callback = [](GLFWwindow* window, int x, int y) ->void {
    width = x;
    height = y;
};
...
void drawline()
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (std::regex_search(objectName, e))
        {
            ...
            ImGui::GetForegroundDrawList()->AddText({ ckzpm.x + (+ckzpm.w / 2) ,ckzpm.y + ckzpm.z - 15.0f }, ImColor(255, 0, 0), reinterpret_cast<const char*>(u8"死鬼"));
        ...
        {
            ImGui::GetForegroundDrawList()->AddText({ x,y }, ImColor(0, 255, 239), "");
        }
